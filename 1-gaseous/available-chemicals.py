#
# Locate all viable CHEMICAL species in the CHEMICALS_DIR
#
import os
import sys
import fnmatch
...
cli_dynamic_names_parser = cli_parser.add_argument_group('Dynamic list of CHEMICAL names', 'Use of these options controls the dynamic build of CHEMICAL name list from a directory of .xyz coordinate files.  Exclude/include rules are applied in the order they are specified.')
...
cli_dynamic_names_parser.add_argument('--chemicals-dir', '-C', metavar='<directory>',
        dest='chemicals_dir',
        default=os.getenv('CHEMICALS_DIR', None),
        help='Search this directory for CHEMICAL species (default: {:s})'.format(os.getenv('CHEMICALS_DIR', 'n/a')))
...
if cli_args.txt_name_list or cli_args.json_name_list:
    ...
    # Does the chemicals directory exist?
    if not os.path.isdir(cli_args.chemicals_dir):
        sys.stderr.write('ERROR:  the chemicals directory `{:s}` does not exist\n'.format(cli_args.chemicals_dir))
        sys.exit(1)
    
    # Look for directories under the chemicals directory:
    for root_dir, sub_dirs, files in os.walk(cli_args.chemicals_dir):
        for CHEMICAL in files:
            if CHEMICAL.endswith('.xyz'):
                # Drop that .xyz suffix:
                CHEMICAL = CHEMICAL[:-4]
                if cli_args.filters:
                    # Initial disposition:
